...
export default function DashboardClient({ products, clients, isClientUser }: DashboardClientProps) {
  ...
  const [selectedClientId, setSelectedClientId] = useState<string>(
    isClientUser && clients.length === 1 ? String(clients[0].id) : ''
  )
  ...
  async function handleConfirmOrder() {
    ...
    try {
      ...
      setCart(new Map())
      setSelectedClientId(isClientUser && clients.length === 1 ? String(clients[0].id) : '')
      router.refresh()
    } catch {
      setShowModal(false)
        ...
        <div className="flex items-center justify-between mb-4 gap-3 flex-wrap">
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="relative">
              <svg className="absolute left-2.5 top-1/2 -translate-y-1/2 w-4 h-4 text-brand-dark/40 pointer-events-none" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-brand-dark/10 bg-brand-dark/5">
                      ...
                      <th className="text-left px-4 py-3 font-medium text-brand-dark/60 hidden sm:table-cell">Type</th>
                      <th className="text-right px-4 py-3 font-medium text-brand-dark/60">Stock</th>
        ...
        <div className="card sticky top-24">
          ...
          {cart.size === 0 ? (
            <div className="text-center py-8">
              <svg className="w-12 h-12 text-brand-dark/25 mx-auto mb-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div className="space-y-2 mb-4 max-h-64 overflow-y-auto">
              {Array.from(cart.values()).map((item) => (
                <div key={item.product.id} className="flex items-center justify-between py-2 border-b border-brand-dark/10 last:border-0">
                  <div className="flex-1 min-w-0">
          ...
          {isClientUser ? (
            clients.length === 1 && (
              <div className="mb-4 px-3 py-2 rounded-lg bg-brand-dark/5 text-sm text-brand-dark/70">
                Ordering as <span className="font-medium text-brand-dark">{clients[0].name} {clients[0].surname}</span>
